...
export const fetchTeams = async (seasonYear?: string): Promise<Team[]> => {
  ...
  // 3. Fetch drivers mapping for this season to relate race_results to their respective teams
  let driversQuery = supabase.from('season_drivers').select('driver_id, team_id');
  if (seasonId) {
    driversQuery = driversQuery.eq('season_id', seasonId);
  }
  const { data: seasonDriversData } = await driversQuery;
  const driverToTeamMap: Record<number, number> = {};
  (seasonDriversData || []).forEach((sd: any) => {
    driverToTeamMap[Number(sd.driver_id)] = Number(sd.team_id);
  });

  // 4. Fetch dynamic race results to calculate actual earned points
  let raceResults: any[] = [];
  if (seasonId) {
    const { data: seasonRacesData } = await supabase
      .from('season_races')
      .select('race_id')
      .eq('season_id', seasonId);
    
    const raceIds = (seasonRacesData || []).map((sr: any) => sr.race_id);
...
export const fetchDrivers = async (seasonYear?: string): Promise<Driver[]> => {
  ...
  // 3. Fetch dynamic race results to calculate actual earned points
  let raceResults: any[] = [];
  if (seasonId) {
    const { data: seasonRacesData } = await supabase
      .from('season_races')
